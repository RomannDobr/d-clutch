...
int main()
{
    ...
    //// остановился на: 3 (стр 956 (в рус 1013))
...
void changeCardValue(string dCluthcPath, string *events, int j, bool plusminus, int day, int month, int year)
{
    ...
    if (quest > 0 && quest <= j)
    {
        ifstream purchaseFile(fs::path(dCluthcPath).replace_filename("d-clutch_data.txt"));
        ...
        ofstream purchaseFile1(fs::path(dCluthcPath).replace_filename("d-clutch_data.txt"));
        bool flag = true;
        for (int i{}; i < k; i++)
        {
